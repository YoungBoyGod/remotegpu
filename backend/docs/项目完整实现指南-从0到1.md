# RemoteGPU 项目完整实现指南 - 从0到1

> **文档版本**: v1.0
> **更新时间**: 2026-02-01
> **项目状态**: 核心功能已完成,持续迭代中
> **适用对象**: 开发者、运维人员、项目管理者

---

## 📋 文档目录

1. [项目概述](#1-项目概述)
2. [技术栈和架构](#2-技术栈和架构)
3. [项目结构](#3-项目结构)
4. [已实现功能清单](#4-已实现功能清单)
5. [从0到1实施步骤](#5-从0到1实施步骤)
6. [数据库设计](#6-数据库设计)
7. [核心模块实现](#7-核心模块实现)
8. [部署和运维](#8-部署和运维)
9. [开发规范](#9-开发规范)
10. [问题排查](#10-问题排查)

---

## 1. 项目概述

### 1.1 项目简介

**RemoteGPU** 是一个云原生的 GPU 资源管理和环境编排平台,旨在为用户提供:

- 🖥️ **按需分配** GPU 计算资源
- 🐳 **容器化环境** 快速创建和管理
- 📊 **资源配额** 灵活的配额管理
- 🔐 **安全访问** 多种远程访问方式
- 📈 **监控告警** 实时资源监控

### 1.2 核心功能

1. **用户管理**: 注册、登录、权限控制
2. **资源管理**: 主机、GPU 资源的注册和分配
3. **环境管理**: 容器化环境的创建、启动、停止、删除
4. **配额管理**: 用户级别的资源配额控制
5. **远程访问**: SSH、RDP、VNC、Jupyter 等多种访问方式
6. **K8s 集成**: 基于 Kubernetes 的容器编排

### 1.3 项目目标

- ✅ 提供简单易用的 GPU 资源管理平台
- ✅ 支持多种部署模式(K8s、Docker、VM)
- ✅ 实现资源的高效利用和公平分配
- ✅ 提供完善的监控和告警机制

---

## 2. 技术栈和架构

### 2.1 后端技术栈

| 技术 | 版本 | 用途 |
|------|------|------|
| Go | 1.21+ | 主要开发语言 |
| Gin | v1.9+ | Web 框架 |
| GORM | v1.25+ | ORM 框架 |
| PostgreSQL | 17.7 | 主数据库 |
| Redis | 7.0+ | 缓存和会话 |
| Kubernetes | 1.28+ | 容器编排 |
| Docker | 24.0+ | 容器运行时 |

### 2.2 架构设计

```
┌─────────────────────────────────────────────────────────┐
│                      API Gateway                         │
│                    (Gin Framework)                       │
└─────────────────────────────────────────────────────────┘
                            │
        ┌───────────────────┼───────────────────┐
        │                   │                   │
┌───────▼────────┐  ┌──────▼──────┐  ┌────────▼────────┐
│  Controller    │  │ Middleware  │  │    Router       │
│  (API Layer)   │  │  (Auth/Log) │  │  (URL Mapping)  │
└───────┬────────┘  └─────────────┘  └─────────────────┘
        │
┌───────▼────────────────────────────────────────────────┐
│              Service Layer (Business Logic)            │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ │
│  │   User   │ │   Host   │ │   GPU    │ │   Env    │ │
│  │ Service  │ │ Service  │ │ Service  │ │ Service  │ │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘ │
└───────┬────────────────────────────────────────────────┘
        │
┌───────▼────────────────────────────────────────────────┐
│              DAO Layer (Data Access)                   │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ │
│  │ User DAO │ │ Host DAO │ │ GPU DAO  │ │ Env DAO  │ │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘ │
└───────┬────────────────────────────────────────────────┘
        │
┌───────▼────────────────────────────────────────────────┐
│                   Database Layer                       │
│  ┌──────────────┐              ┌──────────────┐       │
│  │ PostgreSQL   │              │    Redis     │       │
│  │  (主数据库)   │              │   (缓存)      │       │
│  └──────────────┘              └──────────────┘       │
└────────────────────────────────────────────────────────┘
```

### 2.3 模块化设计

项目采用模块化设计,核心模块包括:

- **pkg/**: 可复用的基础包
  - `cache`: 缓存管理(Redis/Memory)
  - `database`: 数据库连接和迁移
  - `k8s`: Kubernetes 客户端封装
  - `host`: 主机选择和健康检查
  - `network`: 网络和端口管理
  - `volume`: 存储卷管理
  - `deployment`: 部署管理
  - `security`: 安全和认证

---

## 3. 项目结构

### 3.1 目录结构

```
remotegpu/backend/
├── cmd/                    # 应用程序入口
│   └── main.go            # 主程序
├── config/                # 配置文件
│   └── config.yaml        # 主配置文件
├── internal/              # 内部代码(不对外暴露)
│   ├── controller/        # API 控制器层
│   │   └── v1/           # API v1 版本
│   ├── dao/              # 数据访问层
│   ├── middleware/       # 中间件
│   ├── model/            # 数据模型
│   │   └── entity/       # 实体定义
│   ├── router/           # 路由配置
│   └── service/          # 业务逻辑层
├── pkg/                   # 可复用的公共包
│   ├── cache/            # 缓存管理
│   ├── database/         # 数据库连接
│   ├── deployment/       # 部署管理
│   ├── host/             # 主机管理
│   ├── k8s/              # K8s 客户端
│   ├── network/          # 网络管理
│   ├── security/         # 安全认证
│   └── volume/           # 存储管理
├── migrations/           # 数据库迁移脚本
├── sql/                  # SQL 初始化脚本
├── docs/                 # 项目文档
└── k8s/                  # K8s 配置文件
```

### 3.2 核心文件说明

| 文件/目录 | 说明 |
|----------|------|
| `cmd/main.go` | 应用程序入口,初始化各个模块 |
| `config/config.yaml` | 配置文件,包含数据库、Redis、JWT等配置 |
| `internal/router/router.go` | 路由配置,定义所有API端点 |
| `internal/middleware/` | 中间件,包括认证、日志、CORS等 |
| `pkg/database/database.go` | 数据库连接和初始化 |
| `pkg/k8s/client.go` | Kubernetes 客户端封装 |
| `migrations/` | 数据库迁移脚本,记录数据库结构变更 |

---

## 4. 已实现功能清单

### 4.1 用户管理模块 ✅

**完成度**: 90%

#### 实体层
- ✅ User 实体模型 (`internal/model/entity/user.go`)
- ✅ 字段: ID, Username, Email, Password, Role, Status

#### 数据访问层
- ✅ UserDao - 完整的 CRUD 操作
- ✅ 按用户名/邮箱查询
- ✅ 分页列表查询
- ✅ 状态管理

#### 业务逻辑层
- ✅ UserService - 用户注册、登录
- ✅ JWT Token 生成和验证
- ✅ 密码加密(bcrypt)
- ✅ 用户信息获取和更新

#### API 层
- ✅ POST `/api/v1/users/register` - 用户注册
- ✅ POST `/api/v1/users/login` - 用户登录
- ✅ GET `/api/v1/users/me` - 获取当前用户信息
- ✅ PUT `/api/v1/users/me` - 更新用户信息

### 4.2 资源管理模块 ✅

**完成度**: 85%

#### 主机管理
- ✅ Host 实体模型 (`internal/model/entity/host.go`)
- ✅ HostDao - CRUD、状态管理、心跳更新
- ✅ HostService - 主机注册、心跳、资源统计
- ✅ 主机选择策略 (`pkg/host/selector.go`)
- ✅ 健康检查机制 (`pkg/host/health_checker.go`)

**API 端点**:
- ✅ POST `/api/v1/hosts` - 注册主机
- ✅ GET `/api/v1/hosts` - 列出主机
- ✅ GET `/api/v1/hosts/:id` - 获取主机详情
- ✅ PUT `/api/v1/hosts/:id` - 更新主机
- ✅ POST `/api/v1/hosts/:id/heartbeat` - 心跳上报

#### GPU 管理
- ✅ GPU 实体模型 (`internal/model/entity/gpu.go`)
- ✅ GPUDao - CRUD、状态管理、分配/释放
- ✅ GPUService - GPU 注册、分配、释放
- ✅ GPU 状态跟踪(available/allocated/error)

**API 端点**:
- ✅ POST `/api/v1/gpus` - 注册 GPU
- ✅ GET `/api/v1/gpus` - 列出 GPU
- ✅ GET `/api/v1/gpus/:id` - 获取 GPU 详情
- ✅ PUT `/api/v1/gpus/:id/status` - 更新 GPU 状态

### 4.3 环境管理模块 ✅

**完成度**: 80%

#### 核心功能
- ✅ Environment 实体模型 (`internal/model/entity/environment.go`)
- ✅ EnvironmentDao - CRUD 操作
- ✅ EnvironmentService - 环境生命周期管理
- ✅ 多种部署模式支持(K8s Pod/Docker/VM)
- ✅ GPU 自动分配和释放
- ✅ 端口映射管理
- ✅ 访问信息生成

#### 支持的环境类型
- ✅ IDE 环境(SSH + Jupyter)
- ✅ Terminal 环境(SSH)
- ✅ Desktop 环境(SSH + RDP + VNC)
- ✅ Training 环境(SSH + TensorBoard)

**API 端点**:
- ✅ POST `/api/v1/environments` - 创建环境
- ✅ GET `/api/v1/environments` - 列出环境
- ✅ GET `/api/v1/environments/:id` - 获取环境详情
- ✅ DELETE `/api/v1/environments/:id` - 删除环境
- ✅ POST `/api/v1/environments/:id/start` - 启动环境
- ✅ POST `/api/v1/environments/:id/stop` - 停止环境
- ✅ GET `/api/v1/environments/:id/access` - 获取访问信息

### 4.4 资源配额模块 ✅

**完成度**: 95%

#### 核心功能
- ✅ ResourceQuota 实体模型
- ✅ 用户级别配额管理
- ✅ 配额检查(CPU/Memory/GPU/Storage)
- ✅ 已使用资源统计
- ✅ 可用配额计算
- ✅ 并发安全的配额检查(悲观锁)

**API 端点**:
- ✅ POST `/api/v1/quotas` - 设置配额
- ✅ GET `/api/v1/quotas` - 获取配额
- ✅ PUT `/api/v1/quotas/:id` - 更新配额
- ✅ DELETE `/api/v1/quotas/:id` - 删除配额
- ✅ GET `/api/v1/quotas/usage` - 获取资源使用情况

---

## 5. 从0到1实施步骤

### 5.1 环境准备

#### 第一步:安装基础软件

```bash
# 1. 安装 Go 1.21+
wget https://go.dev/dl/go1.21.0.linux-amd64.tar.gz
sudo tar -C /usr/local -xzf go1.21.0.linux-amd64.tar.gz
export PATH=$PATH:/usr/local/go/bin

# 2. 安装 PostgreSQL 17
sudo apt-get install postgresql-17

# 3. 安装 Redis
sudo apt-get install redis-server

# 4. 安装 Docker (可选)
curl -fsSL https://get.docker.com | sh

# 5. 安装 Kubernetes (可选)
# 参考官方文档安装 kubectl 和 minikube/k3s
```

#### 第二步:配置数据库

```bash
# 创建数据库和用户
sudo -u postgres psql
CREATE DATABASE remotegpu;
CREATE USER remotegpu_user WITH PASSWORD 'remotegpu_password';
GRANT ALL PRIVILEGES ON DATABASE remotegpu TO remotegpu_user;
\q
```

### 5.2 项目初始化

#### 第一步:创建项目结构

```bash
# 创建项目目录
mkdir -p remotegpu/backend
cd remotegpu/backend

# 初始化 Go 模块
go mod init github.com/YoungBoyGod/remotegpu

# 创建目录结构
mkdir -p cmd config internal/{controller/v1,dao,middleware,model/entity,router,service}
mkdir -p pkg/{auth,cache,database,deployment,errors,host,k8s,network,security,volume}
mkdir -p migrations sql docs k8s logs
```

#### 第二步:创建配置文件

创建 `config/config.yaml`:

```yaml
server:
  port: 8080
  mode: debug

database:
  host: localhost
  port: 5432
  user: remotegpu_user
  password: remotegpu_password
  dbname: remotegpu

redis:
  host: localhost
  port: 6379
  password: ""
  db: 0
  pool_size: 10
  timeout: 5

jwt:
  secret: "your-secret-key-change-in-production"
  expire_time: 24

log:
  level: debug
  output_path: "./logs/app.log"
```

---

### 5.3 核心模块开发

#### 第一步:基础设施层 (pkg/)

**1. 数据库连接** (`pkg/database/database.go`)

```go
package database

import (
    "gorm.io/driver/postgres"
    "gorm.io/gorm"
)

var db *gorm.DB

func InitDB(dsn string) error {
    var err error
    db, err = gorm.Open(postgres.Open(dsn), &gorm.Config{})
    return err
}

func GetDB() *gorm.DB {
    return db
}
```

**2. 缓存管理** (`pkg/cache/cache.go`)

```go
package cache

import (
    "context"
    "time"
    "github.com/redis/go-redis/v9"
)

type CacheManager struct {
    caches map[string]Cache
}

func NewCacheManager() *CacheManager {
    return &CacheManager{caches: make(map[string]Cache)}
}
```

**3. K8s 客户端** (`pkg/k8s/client.go`)

```go
package k8s

import (
    "k8s.io/client-go/kubernetes"
    "k8s.io/client-go/rest"
)

func GetClient() (*kubernetes.Clientset, error) {
    config, err := rest.InClusterConfig()
    if err != nil {
        return nil, err
    }
    return kubernetes.NewForConfig(config)
}
```

#### 第二步:数据模型层 (internal/model/entity/)

**创建实体模型**:

```go
// internal/model/entity/user.go
type User struct {
    ID        uint      `gorm:"primarykey"`
    Username  string    `gorm:"unique;not null"`
    Email     string    `gorm:"unique;not null"`
    Password  string    `gorm:"not null"`
    Role      string    `gorm:"default:'user'"`
    CreatedAt time.Time
    UpdatedAt time.Time
}

// internal/model/entity/host.go
type Host struct {
    ID          string `gorm:"primarykey"`
    Name        string
    IPAddress   string
    Status      string
    TotalCPU    int
    UsedCPU     int
    TotalMemory int64
    UsedMemory  int64
    TotalGPU    int
    UsedGPU     int
}

// internal/model/entity/environment.go
type Environment struct {
    ID              string `gorm:"primarykey"`
    UserID          uint
    HostID          string
    Name            string
    Image           string
    Status          string
    CPU             int
    Memory          int64
    GPU             int
    DeploymentMode  string
    EnvironmentType string
}
```

---

### 5.4 数据库初始化

#### 第一步:创建数据库表

创建 `sql/03_users_and_permissions.sql`:

```sql
-- 用户表
CREATE TABLE IF NOT EXISTS users (
    id BIGSERIAL PRIMARY KEY,
    username VARCHAR(64) UNIQUE NOT NULL,
    email VARCHAR(128) UNIQUE NOT NULL,
    password VARCHAR(256) NOT NULL,
    role VARCHAR(32) DEFAULT 'user',
    status VARCHAR(32) DEFAULT 'active',
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW()
);

-- 资源配额表
CREATE TABLE IF NOT EXISTS resource_quotas (
    id BIGSERIAL PRIMARY KEY,
    user_id BIGINT NOT NULL,
    cpu INT DEFAULT 0,
    memory BIGINT DEFAULT 0,
    gpu INT DEFAULT 0,
    storage BIGINT DEFAULT 0,
    environment_quota INT DEFAULT 0,
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
    FOREIGN KEY (user_id) REFERENCES users(id)
);
```

创建 `sql/04_hosts_and_devices.sql`:

```sql
-- 主机表
CREATE TABLE IF NOT EXISTS hosts (
    id VARCHAR(64) PRIMARY KEY,
    name VARCHAR(128) NOT NULL,
    ip_address VARCHAR(64) NOT NULL,
    status VARCHAR(32) DEFAULT 'active',
    total_cpu INT DEFAULT 0,
    used_cpu INT DEFAULT 0,
    total_memory BIGINT DEFAULT 0,
    used_memory BIGINT DEFAULT 0,
    total_gpu INT DEFAULT 0,
    used_gpu INT DEFAULT 0,
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW()
);

-- GPU表
CREATE TABLE IF NOT EXISTS gpus (
    id BIGSERIAL PRIMARY KEY,
    host_id VARCHAR(64) NOT NULL,
    gpu_index INT NOT NULL,
    model VARCHAR(128),
    memory BIGINT,
    status VARCHAR(32) DEFAULT 'available',
    allocated_to VARCHAR(64),
    FOREIGN KEY (host_id) REFERENCES hosts(id)
);
```

#### 第二步:执行数据库初始化

```bash
# 执行SQL脚本
psql -h localhost -U remotegpu_user -d remotegpu -f sql/03_users_and_permissions.sql
psql -h localhost -U remotegpu_user -d remotegpu -f sql/04_hosts_and_devices.sql
psql -h localhost -U remotegpu_user -d remotegpu -f sql/05_environments.sql
```

### 5.5 启动和测试

#### 第一步:安装依赖

```bash
go mod tidy
```

#### 第二步:启动应用

```bash
go run cmd/main.go
```

#### 第三步:测试API

```bash
# 1. 用户注册
curl -X POST http://localhost:8080/api/v1/users/register \
  -H "Content-Type: application/json" \
  -d '{"username":"test","email":"test@example.com","password":"123456"}'

# 2. 用户登录
curl -X POST http://localhost:8080/api/v1/users/login \
  -H "Content-Type: application/json" \
  -d '{"username":"test","password":"123456"}'

# 3. 注册主机
curl -X POST http://localhost:8080/api/v1/hosts \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"id":"host-001","name":"GPU Server 1","ip_address":"192.168.1.100"}'
```

---

## 6. 数据库设计

### 6.1 核心表结构

#### users (用户表)
- id: 主键
- username: 用户名(唯一)
- email: 邮箱(唯一)
- password: 密码(bcrypt加密)
- role: 角色(user/admin)
- status: 状态(active/inactive)

#### hosts (主机表)
- id: 主键
- name: 主机名称
- ip_address: IP地址
- total_cpu/used_cpu: CPU资源
- total_memory/used_memory: 内存资源
- total_gpu/used_gpu: GPU资源

#### environments (环境表)
- id: UUID主键
- user_id: 用户ID(外键)
- host_id: 主机ID(外键)
- name: 环境名称
- image: 镜像名称
- status: 状态(creating/running/stopped)
- cpu/memory/gpu: 资源配置

#### resource_quotas (资源配额表)
- id: 主键
- user_id: 用户ID(外键)
- cpu/memory/gpu/storage: 资源配额
- environment_quota: 环境数量配额

### 6.2 表关系
- users (1:N) environments
- users (1:1) resource_quotas
- hosts (1:N) environments
- hosts (1:N) gpus

---

## 7. 部署和运维

### 7.1 Docker 部署

**Dockerfile**:
```dockerfile
FROM golang:1.21-alpine AS builder
WORKDIR /app
COPY . .
RUN go mod download
RUN go build -o main cmd/main.go

FROM alpine:latest
WORKDIR /app
COPY --from=builder /app/main .
COPY --from=builder /app/config ./config
EXPOSE 8080
CMD ["./main"]
```

**构建和运行**:
```bash
docker build -t remotegpu-backend .
docker run -p 8080:8080 remotegpu-backend
```

### 7.2 Docker Compose 部署

**docker-compose.yml**:
```yaml
version: '3.8'
services:
  postgres:
    image: postgres:17
    environment:
      POSTGRES_DB: remotegpu
      POSTGRES_USER: remotegpu_user
      POSTGRES_PASSWORD: remotegpu_password
    ports:
      - "5432:5432"

  redis:
    image: redis:7-alpine
    ports:
      - "6379:6379"

  backend:
    build: .
    ports:
      - "8080:8080"
    depends_on:
      - postgres
      - redis
```

**启动**:
```bash
docker-compose up -d
```

---

## 8. 项目总结

### 8.1 已完成的核心功能

✅ **用户管理系统** (90%)
- 用户注册、登录、JWT认证
- 角色权限管理

✅ **资源管理系统** (85%)
- 主机注册和管理
- GPU资源分配和释放
- 主机健康检查和选择策略

✅ **环境管理系统** (80%)
- 容器化环境创建和管理
- 多种部署模式(K8s/Docker)
- 多种环境类型(IDE/Terminal/Desktop/Training)
- GPU自动分配

✅ **资源配额系统** (95%)
- 用户级别配额管理
- 配额检查和统计
- 并发安全保证

✅ **基础设施** (100%)
- 数据库连接和迁移
- Redis缓存支持
- K8s客户端封装
- 模块化架构设计

### 8.2 项目里程碑

| 阶段 | 完成时间 | 主要成果 |
|------|---------|---------|
| 阶段1 | 2026-01 | 项目初始化、用户管理、基础设施 |
| 阶段2 | 2026-01 | 资源管理、主机和GPU管理 |
| 阶段3 | 2026-01 | 环境管理、K8s集成 |
| 阶段4 | 2026-02 | 资源配额、数据库优化、Workspace移除 |

### 8.3 技术亮点

1. **模块化架构**: 清晰的分层设计,易于维护和扩展
2. **并发安全**: 使用悲观锁保证配额检查的并发安全
3. **多部署模式**: 支持K8s、Docker等多种部署方式
4. **缓存策略**: Redis缓存+MemoryCache fallback
5. **健康检查**: 主机健康监控和自动选择

### 8.4 下一步计划

📋 **待完成功能**:
- [ ] 镜像管理模块
- [ ] 数据集管理模块
- [ ] 监控告警系统
- [ ] 计费管理模块
- [ ] 完善的日志系统
- [ ] 性能优化和压力测试

🔧 **技术优化**:
- [ ] 启动HealthMonitor
- [ ] 完善主机数据同步机制
- [ ] 统一日志框架
- [ ] 优雅关闭机制

---

## 9. 快速开始指南

### 一键启动(推荐)

```bash
# 1. 克隆项目
git clone https://github.com/YoungBoyGod/remotegpu.git
cd remotegpu/backend

# 2. 启动依赖服务
docker-compose up -d postgres redis

# 3. 初始化数据库
psql -h localhost -U remotegpu_user -d remotegpu -f sql/03_users_and_permissions.sql
psql -h localhost -U remotegpu_user -d remotegpu -f sql/04_hosts_and_devices.sql
psql -h localhost -U remotegpu_user -d remotegpu -f sql/05_environments.sql

# 4. 启动应用
go run cmd/main.go
```

### 验证部署

```bash
# 检查服务状态
curl http://localhost:8080/health

# 注册测试用户
curl -X POST http://localhost:8080/api/v1/users/register \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","email":"admin@example.com","password":"admin123"}'
```

---

## 10. 参考资源

### 项目文档
- [功能清单和依赖关系分析](./功能清单和依赖关系分析.md)
- [模块化架构设计](./模块化架构设计.md)
- [API文档](./api/)

### 技术文档
- [Go官方文档](https://go.dev/doc/)
- [Gin框架文档](https://gin-gonic.com/docs/)
- [GORM文档](https://gorm.io/docs/)
- [Kubernetes文档](https://kubernetes.io/docs/)

---

**文档结束**

> 本文档持续更新中,如有问题请提交Issue或联系项目维护者。
> 
> 最后更新: 2026-02-01
> 维护者: RemoteGPU Team

